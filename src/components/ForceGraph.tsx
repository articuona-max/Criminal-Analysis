...
export const ForceGraph: React.FC<ForceGraphProps> = ({
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[calc(100vh-140px)] min-h-[600px] bg-slate-100/60 overflow-hidden flex flex-col">
      
      {/* SVG Canvas */}
      <svg ref={svgRef} className="w-full h-full cursor-grab active:cursor-grabbing" />

      {/* Top Floating Controls Bar */}
      <div className="absolute top-4 left-4 z-20 flex flex-wrap items-center gap-2 bg-white/95 backdrop-blur-md border border-slate-200 p-2 rounded-2xl text-xs shadow-md">
        
        {/* Color Mode Switcher */}
        <div className="flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl border border-slate-200">
          <span className="text-[10px] text-slate-500 uppercase px-1.5 font-semibold">Mode:</span>
          {(['POLE', 'COMMUNITY', 'RISK', 'ANOMALY', 'BROKER'] as GraphColorMode[]).map(mode => (
            <button
              key={mode}
        ...
        <div className="flex items-center gap-1 pl-1 border-l border-slate-200">
          <button
            ...
            className="p-1.5 rounded-xl bg-white text-slate-600 hover:text-slate-900 hover:bg-slate-50 border border-slate-200 shadow-2xs"
            title="Zoom In"
          >
